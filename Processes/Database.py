...
async def getLocalAnswer(queryData):
    ...
    try:
        ...
        async with engine.connect() as conn:
            s = select(domains2.c.answer).filter(
                domains2.c.domain == queryData.question[0].to_text()
            )
...
async def getRemoteAnswer(queryData):
    ...
    """
    useServer = {"ip": "0.0.0.0", "port": 53}
    for backupServerName in si.info["Dnsserver"]["Backupservers"].keys():
        if si.info["Dnsserver"]["Backupservers"][backupServerName]["enabled"]:
            useServer["ip"] = si.info["Dnsserver"]["Backupservers"][backupServerName][
                "ip"
            ]
            useServer["port"] = si.info["Dnsserver"]["Backupservers"][backupServerName][
                "port"
            ]
            break
    forwardedResponse = dns.query.udp(
        queryData, useServer["ip"], useServer["port"])
    logging.debug(
        f"[+] Forwarded query to {useServer['ip']}:{useServer['port']}")
    addAnswer(queryData.question[0].to_text(), str(forwardedResponse))
    logging.debug(
        f"[+] Found answer in remote nameserver and added to local database")
